...
@onready var onlineIndicatorText=$Offline_Online
@export var onlineMode=false

var receivedScoreboardFromNetwork=false
...
func _process(delta: float) -> void:
	if !receivedScoreboardFromNetwork:
		loadScores("scores.save",onlineMode)
		updateScoreboard()
	else:
		showLineEdit()
func sort():
...
func saveScores(filepath:String,online:bool):
	if online:
		var sendScoreboard=""
...
func loadScores(filepath : String,online:bool):
	if online:
		...
		if data=="":
			...
			receivedScoreboardFromNetwork=true
...
func _on_line_edit_text_submitted(new_text: String) -> void:
	...
	saveScores("scores.save",onlineMode)
